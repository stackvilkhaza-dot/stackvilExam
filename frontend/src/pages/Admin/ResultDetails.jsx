...
const ResultDetails = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tab Switch History Log */}
      {result.tabSwitches && result.tabSwitches.length > 0 && (
        <div className="bg-red-50 rounded-xl shadow-sm border border-red-200 p-6">
          <div className="flex items-center space-x-2 text-red-800 font-bold mb-3">
            <svg className="w-6 h-6 animate-pulse text-red-650" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
            <span className="text-red-900">Proctoring Alert: Candidate Switched Tabs ({result.tabSwitches.length} times)</span>
          </div>
          <div className="max-h-40 overflow-y-auto border border-red-200 rounded-lg bg-white p-3 space-y-2">
            {result.tabSwitches.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm border-b pb-1.5 last:border-0 last:pb-0">
                <span className="text-red-700 font-medium">⚠️ Tab switched</span>
                <span className="text-gray-500 text-xs">Round: {item.round || 'N/A'} at {item.time}</span>
              </div>
            ))}
          </div>
        </div>
